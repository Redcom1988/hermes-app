...
object CreateClientScreen : Screen {

    @OptIn(ExperimentalMaterial3Api::class)
    @Composable
    override fun Content() {
        val navigator = LocalNavigator.currentOrThrow
        val screenModel = rememberScreenModel { CreateClientScreenModel() }
        val state by screenModel.state.collectAsState()

        // Navigate back when client is created successfully
        LaunchedEffect(state.isClientCreated) {
            if (state.isClientCreated) {
                navigator.pop()
            }
        }
        ...
        FormLayout(
            ...
            LazyColumn(
                ...
                item {
                    Button(
                        onClick = { screenModel.createClient() },
                        modifier = Modifier
                            .fillMaxWidth()
                            .height(56.dp),
