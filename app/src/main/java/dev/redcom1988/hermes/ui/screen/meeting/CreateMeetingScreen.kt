...
object CreateMeetingScreen : Screen {

    @OptIn(ExperimentalMaterial3Api::class)
    @Composable
    override fun Content() {
        val navigator = LocalNavigator.currentOrThrow
        val screenModel = rememberScreenModel { CreateMeetingScreenModel() }
        val state by screenModel.state.collectAsState()

        // Navigate back when meeting is created successfully
        LaunchedEffect(state.isMeetingCreated) {
            if (state.isMeetingCreated) {
                navigator.pop()
            }
        }
        ...
        FormLayout(
            ...
            LazyColumn(
                ...
                item {
                    Button(
                        onClick = { screenModel.createMeeting() },
                        modifier = Modifier
                            .fillMaxWidth()
                            .height(56.dp),
