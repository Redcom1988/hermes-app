...
class MeetingApiImpl(
    ...
    override suspend fun getMeetingData(): Response {
        val request = GET("$baseUrl/meetings")
        return networkHelper.client.newCall(request).await()
    }

    override suspend fun pushMeetingChanges(request: MeetingApiRequestDto): Response {
        val json = Json.encodeToString(MeetingApiRequestDto.serializer(), request)
        val requestBody = json.toRequestBody("application/json".toMediaType())
        val requestObj = POST("$baseUrl/meetings/sync", body = requestBody)
